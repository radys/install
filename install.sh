...
# Povolení openssh serveru po rebootu
systemctl enable ssh

# Vložení authorized_keys do souboru v .ssh roota
mkdir /root/.ssh
...
# Odkomentování přihlášení roota přes klíč v sshd_config a zakázání přihlášení heslem
sed -ri "s/^#? *PasswordAuthentication *yes.*/PasswordAuthentication no/" /etc/ssh/sshd_config
sed -i "s/.*PermitRootLogin.*/PermitRootLogin without-password/g" /etc/ssh/sshd_config

# Restart openssh serveru
